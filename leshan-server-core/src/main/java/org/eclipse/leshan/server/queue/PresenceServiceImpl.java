...
public final class PresenceServiceImpl implements PresenceService, Destroyable {
    ...
    private final ConcurrentMap<String /* endpoint */, AtomicReference<ScheduledFuture<?>>> clientPrecenses = new ConcurrentHashMap<>();
    ...
    public boolean isClientAwake(Registration registration) {
        return clientPrecenses.get(registration.getEndpoint()) != null;
    }
    ...
    public void setAwake(final Registration reg) {
        if (reg.usesQueueMode()) {
            ...
            AtomicReference<ScheduledFuture<?>> previous = clientPrecenses.put(reg.getEndpoint(), timerFuture);
            if (previous != null) {
                stateChanged = false;
            ...
            if (clientAwakeTime != 0) {
                timerFuture.set(clientTimersExecutor.schedule(new Runnable() {
                    @Override
                    public void run() {
                        boolean removed = clientPrecenses.remove(reg.getEndpoint(), timerFuture);
                        if (removed) {
    ...
    public void setSleeping(Registration reg) {
        if (reg.usesQueueMode()) {
            AtomicReference<ScheduledFuture<?>> timerFuture = clientPrecenses.remove(reg.getEndpoint());
            if (timerFuture != null) {
    ...
    public void stopPresenceTracking(Registration reg) {
        clientPrecenses.remove(reg.getEndpoint());
    }
